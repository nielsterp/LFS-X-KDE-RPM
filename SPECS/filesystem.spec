...
192.168.0.110	lfs.example.com lfs
...
EOF
echo '/dev/'${DEVICE}'	/	'${FILESYSTEM}'	defaults 1 1 ' >> %{buildroot}/etc/fstab  
echo '/dev/'${SWAP}'	swap	swap	pri=1	0	0 ' >> %{buildroot}/etc/fstab
cat >> %{buildroot}/etc/fstab <<- "EOF"
...
install ehci_hcd /sbin/modprobe ehci_hcd ; /sbin/modprobe -i ehci_hcd ; true
install xhci_hcd /sbin/modprobe ehci_hcd ; /sbin/modprobe -i xhci_hcd ; true
...
EOF
echo "DISTRIB_RELEASE=7.7 ${OS_bits}" >> %{buildroot}/etc/lsb-release
cat >> %{buildroot}/etc/lsb-release <<- "EOF"
...
%endif
%changelog
*	Fri Aug 28 2015 Niels Terp <nielsterp@comhem.se> 7.7
